...
<div style="max-width: 900px; margin: 0 auto; padding: 40px;">
    ...
    <div style="margin-bottom: 40px;">
        ...
        <h1 style="font-size: 32px; font-weight: 900; color: var(--text); letter-spacing: -1px; margin: 0;">Register New Product</h1>
        <p style="color: var(--muted); margin-top: 8px;">Create a new technical configuration for your product catalog.</p>
    </div>
    ...
    <form action="{{ route('admin.products.store') }}" method="POST" id="product-form">
        ...
        <div style="display: grid; gap: 30px;">
            ...
            <div style="background: #fff; border: 1px solid var(--border); border-radius: 24px; padding: 32px; box-shadow: 0 10px 30px rgba(0,0,0,0.02);">
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 20px;">
                    <div>
                        ...
                        <input type="text" name="name" required placeholder="e.g. Scala Roller Blind" class="form-control-premium">
                    </div>
                    <div>
                        <label class="form-label-premium">Category</label>
                        <input type="text" name="category" placeholder="e.g. Blinds, Windows" class="form-control-premium">
                    </div>
                </div>

                <div style="margin-bottom: 20px;">
                    <label class="form-label-premium">Description</label>
                    <textarea name="description" rows="3" placeholder="Technical description of the product..." class="form-control-premium" style="resize: vertical;"></textarea>
                </div>
            </div>
                ...
                <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
                    <div>
                        ...
                        <select name="pricing_type" class="form-control-premium">
                            ...
                            <option value="area">Area Based (Sqft/Sqm)</option>
                        </select>
                    </div>
                    <div>
                        <label class="form-label-premium">Base Unit Price ($)</label>
                        <input type="number" step="0.01" name="unit_price" required placeholder="0.00" class="form-control-premium">
                    </div>
                </div>
            </div>
            ...
            <div style="display: flex; justify-content: flex-end; gap: 16px; margin-top: 20px; padding-bottom: 60px;">
                <a href="{{ route('admin.products.index') }}" style="padding: 16px 32px; border-radius: 16px; background: #fff; border: 1px solid var(--border); color: var(--muted); font-weight: 700; text-decoration: none; display: flex; align-items: center;">Cancel</a>
                <button type="submit" style="padding: 16px 48px; border-radius: 16px; background: var(--gold); border: none; color: #fff; font-weight: 900; box-shadow: 0 10px 20px -5px rgba(184, 155, 94, 0.4); cursor: pointer;">Initialize Product ➔</button>
            </div>
